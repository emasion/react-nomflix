...
import {getCredit, getMovie, getSimilar, IGetVideosResult} from "../api";
...
function DetailBox() {
    const movieMatch = useRouteMatch<{ movieId: string, type: string }>("/movie/:type/:movieId");
    const videoId = movieMatch?.params.movieId || "";
    const type = movieMatch?.params.type || "";
    ...
    } = useQuery<IDetailInfo>(["movie", videoId], () => getMovie(videoId))
    const {
        data: cast,
        isLoading: isLoadingCast
    } = useQuery<ICreditInfo>(["credit", videoId], () => getCredit(videoId))
    const {
        data: similar,
        isLoading: isLoadingSimilar
    } = useQuery<IGetVideosResult>(["similar", videoId], () => getSimilar(videoId))
